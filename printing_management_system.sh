...
approve_reject_prepaid() {
  ...
  case $action in
        1)
            extracted_user=$(grep -B5 "^Transaction ID: $trans_id$" "$PREPAID_REQUESTS" | grep "^Username: " | head -1 | sed 's/^Username: //')
            extracted_pages=$(grep -A1 "^Transaction ID: $trans_id$" "$PREPAID_REQUESTS" | grep "^Pages Requested: " | sed 's/^Pages Requested: //')
            
            echo "${extracted_user}:${trans_id}:${extracted_pages}" >> "$PREPAID_FILE"
...
' "$PREPAID_REQUESTS" > temp.txt && mv temp.txt "$PREPAID_REQUESTS"
            ...
            echo "Prepaid package with Transaction ID $trans_id has been approved."
        ;;
...
check_status() {
    ...
    INPROGRESS_FILE="in_progress_orders.txt"
